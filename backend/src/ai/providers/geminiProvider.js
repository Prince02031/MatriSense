...
/**
 * Same structured-JSON contract as generateJsonWithGemini, but the user
 * turn includes an inline image part (multimodal / vision input).
 */
const generateJsonWithGeminiVision = async ({ systemInstruction, userPrompt, responseSchema, temperature, imageBase64, mimeType }) => {
  const apiKey = process.env.GEMINI_API_KEY?.trim();
  if (!apiKey) {
    throw new Error('[GeminiProvider] GEMINI_API_KEY is missing in environment variables.');
  }
  ...
  try {
    ...
    if (!rawText) {
      console.error('[GeminiProvider] Empty vision response. Raw result:', JSON.stringify(result, null, 2));
      throw new Error('[GeminiProvider] Received empty response from Gemini Vision API.');
    }
    ...
    console.error('[GeminiProvider] Vision SDK Failure:', error);
    throw new Error(`[GeminiProvider] Vision API Failure: ${error.message}`);
  }
};

module.exports = {
  generateJsonWithGemini,
  generateJsonWithGeminiVision
};
